document.addEventListener('DOMContentLoaded', () => {
    ...
    for (var i = 0, len = squares.length; i < len; i++ ) 
        ...
        // check win or lose
        function checkBoard() {
            // array of all winning arrays, array of arrays
            const winningArrays = [
                ...
                // plug the four winning values from the arays to the squares
                for (let y = 0; y < winningArrays.length; y++) {
                    ...
                    // check which player is the player [1 or 2]
                    if (square1.classList.contains('player-one') && 
                        square2.classList.contains('player-one') &&
                        square3.classList.contains('player-one') &&
                        square4.classList.contains('player-one')) {
                        // if it's player one then player one wins
                        result.innerHTML = 'Player One Wins! Hoooray!'
                    } else if (square1.classList.contains('player-one') &&
                        square2.classList.contains('player-one') &&
                        square3.classList.contains('player-one') &&
                        square4.classList.contains('player-one')) {
                            result.innerHTML = 'Player One Wins! Hoooray!'
        ...
        //listen to each square for click events
